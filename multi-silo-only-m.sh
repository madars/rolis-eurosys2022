...
do
  if ((skip_cgroup == 0)); then
    ...
    sudo cgexec -g cpuset:cpulimitl ./out-perf.masstree/benchmarks/dbtest --verbose --bench micro \
                                                                          ...
                                                                          --paxos-leader-config --multi-process -P localhost -S 1000 > ./silo-only-logs-m/leader-$trd-1000.log 2>&1 &
  else
    ./out-perf.masstree/benchmarks/dbtest --verbose --bench micro \
                                                                          --db-type mbta --scale-factor $trd --num-threads $trd \
                                                                          --numa-memory 1G --parallel-loading --runtime 10 \
                                                                          -F third-party/paxos/config/local/1follower_$trd.yml \
                                                                          -F third-party/paxos/config/occ_paxos.yml \
                                                                          --paxos-leader-config --multi-process -P localhost -S 1000 > ./silo-only-logs-m/leader-$trd-1000.log 2>&1 &
